...
$csrf = csrf_token();
render_header(lang_text('pages.free_show.meta.title', 'Just One KISS — July 25, 2026 Free Show'), lang_text('pages.free_show.meta.description', 'Admission is free at Cycle Moore Legacy, 11075 US 31 South, Interlochen, MI 49643. No ticket is required; RSVP is appreciated. The event runs all day; camping is separate.'));
render_notice($formResult);
?>
<main id="main">
  <?php render_page_section_slot('/july-25-2026/', 'before_page'); ?>
  <?php render_page_section_slot('/july-25-2026/', 'before_section_01'); ?>
  <section class="subhero section-shell" aria-labelledby="page-title">
    <p class="eyebrow"><?= lang_editable('pages.free_show.hero.eyebrow', 'Free show · July 25, 2026 · Cycle Moore Legacy') ?></p>
    <h1 id="page-title"><?= lang_editable('pages.free_show.hero.title', 'July 25, 2026 Free Show') ?></h1>
  ...
  <section class="section-shell proof-section" aria-labelledby="known-title">
    ...
    <div class="proof-grid">
      <article><span class="proof-badge"><?= lang_editable('pages.free_show.card.1.badge', '01') ?></span><h3><?= lang_editable('pages.free_show.card.1.title', 'Free show') ?></h3><p><?= lang_editable('pages.free_show.card.1.body', 'The July 25, 2026 Just One KISS show is free. No ticket is required; RSVP is appreciated so updates can find you before show day.') ?></p></article>
  ...
  <?php render_page_section_slot('/july-25-2026/', 'before_section_03'); ?>
  <section class="section-shell date-card" aria-labelledby="expect-title">
    ...
    <?php render_page_image('fog-strobe-atmosphere.webp', lang_text('pages.july_25_2026.image.2.alt', 'Family-friendly blast atmosphere image.')); ?>
  </section>

  <?php render_page_section_slot('/july-25-2026/', 'before_section_04'); ?>
  <section id="updates" class="section-shell form-section" aria-labelledby="updates-title"><div><p class="eyebrow"><?= lang_editable('pages.free_show.updates.eyebrow', 'Do not miss the drop') ?></p><h2 id="updates-title"><?= lang_editable('pages.free_show.updates.title', 'Join the July 25 update list.') ?></h2><p><?= lang_editable('pages.free_show.updates.body', 'Get arrival notes, schedule reminders, camping and parking updates, safety/access notes, Facebook event link updates, and photo/video sharing instructions as they are confirmed.') ?></p><div class="safety-callout"><?= lang_editable_with_strong_prefix('pages.free_show.updates.callout', 'No ticket required: RSVP is appreciated, and email is the cleanest way to keep the details from getting lost in the smoke.', 'No ticket required:') ?></div></div><?php render_lead_form($csrf); ?><?php render_page_image('gear-control-dossier.webp', lang_text('pages.july_25_2026.image.3.alt', 'Update-list form background.')); ?></section>
  <?php render_page_section_slot('/july-25-2026/', 'after_page'); ?>
</main>
<?php render_footer(); ?>
